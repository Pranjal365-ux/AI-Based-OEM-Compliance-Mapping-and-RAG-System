...
#     extraction_model: Optional[str] = "gemma2:9b"

#     # ── Compliance verdict model — used for per-requirement compliance
#     #    judgments where quality matters most. Routes to Groq (free tier)
#     #    for 70B-parameter models that dramatically outperform 7B locals
#     #    on structured judgment tasks.
#     #    Set compliance_provider to "local" to use the local Ollama model instead.
#     compliance_provider: str = "local"

#     compliance_model: str = "gemma2:9b"

#     compliance_base_url: str = "http://100.98.219.69:11434/v1"

#     compliance_api_key: str = "local"
...
#         r'\b[A-Z]{1,4}-\d{3,5}[A-Z](?:-[A-Z0-9]+){1,3}\b',
...
#     # Vendor-specific model name prefixes — used for canonicalization.
#     # Unknown vendors pass through unchanged; no vendor is rejected outright.
#     vendor_model_prefixes: Dict[str, List[str]] = field(default_factory=lambda: {
#         "fortinet":            ["FG-", "FAZ-", "FMG-", "FSA-", "FWB-", "FAD-"],
#         "palo alto networks":  ["PA-", "M-", "WF-"],
#         "cisco":               ["ASA", "FPR", "ISR", "C9", "WS-"],
#         "juniper networks":    ["SRX", "MX-", "EX-", "QFX"],
#         "check point":         ["CPAP-", "CPSB-"],
#         "sonicwall":           ["NSA", "NSsp", "TZ"],
#         "sophos":              ["XGS", "XG-", "SG-"],
#         "aruba":               ["JL-", "R-"],
#     })

#     # Vendor-specific suffixes to strip when deduplicating models.
#     # Maps vendor (lowercase) → list of regex patterns for non-product suffixes.
#     vendor_strip_suffixes: Dict[str, List[str]] = field(default_factory=lambda: {
#         "fortinet":            [r"-(?:FAN|PS|CH|DAT)$"],
#         "palo alto networks":  [r"-(?:ZTP|TAA|GOV)$"],
#     })
...
class LLMConfig:
    ...
    # ── Reasoning model — used for compliance report generation where
    #    chain-of-thought genuinely helps answer quality.
    #    openai/gpt-oss-120b is Groq's flagship open-weight reasoning model —
    #    best quality currently available on Groq, with a generous
    #    250K TPM / 1K RPM rate limit on the developer plan.
    model: str = "qwen/qwen3-32b"
    base_url: str = "https://api.groq.com/openai/v1"
    api_key: str = field(
        default_factory=lambda: os.getenv("GROQ_API_KEY", os.getenv("LLM_API_KEY", "local"))
    )

    # ── Fast extraction model — used for RFP requirement extraction where
    #    we need pure JSON output with no reasoning overhead.
    #    openai/gpt-oss-20b is the fastest production model on Groq
    #    (~1000 tok/s) while still being a strong instruct model — ideal for
    #    high-volume JSON extraction. Shares the same 250K TPM / 1K RPM limit.
    #    Set to None to fall back to `model` above (with /no_think suppression).
    extraction_model: Optional[str] = "qwen/qwen3-32b"

    # ── Compliance verdict model — used for per-requirement compliance
    #    judgments where quality matters most. Routed to Groq for fast,
    #    high-quality structured judgment output.
    #    Set compliance_provider to "local" to use a local Ollama model instead.
    compliance_provider: str = "groq"

    compliance_model: str = "qwen/qwen3-32b"

    compliance_base_url: str = "https://api.groq.com/openai/v1"

    compliance_api_key: str = field(
        default_factory=lambda: os.getenv("GROQ_API_KEY", os.getenv("LLM_API_KEY", "local"))
    )

    # ── Rate limiting (Groq developer-plan defaults) ────────────────────────
    # gpt-oss-120b and gpt-oss-20b both share 250K TPM / 1K RPM. We throttle
    # client-side so we smoothly stay under these instead of bursting into
    # 429s. Tune down if you're on a lower (free) tier.
    rpm_limit: int = 900         # stay safely under the 1K RPM ceiling
    tpm_limit: int = 230000      # stay under 250K TPM with headroom
