...
def extract_header_image(
    ...
    dpi: int = 150,             # 150 DPI is enough to read a company name
) -> Optional[Image.Image]:
    """
...
def detect_vendor_from_text(
    pages: List[dict],
    pages_to_check: int = 2,
) -> Tuple[str, float, str]:
    """
    Fast path: scan already-extracted page text for a known vendor name.
    Checks only the first ``pages_to_check`` pages and only the first
    30 lines of each (the header / title area).

    Returns:
        (vendor_name, confidence, raw_snippet)  — confidence 0.0 if not found.
    """
    snippets: List[str] = []
    for page in pages[:pages_to_check]:
        text = page.get("cleaned_text", "")
        # Only the first 30 lines — the vendor name is almost always in
        # the letterhead / title block at the very top of the page.
        head = "\n".join(text.splitlines()[:30])
        if head.strip():
            snippets.append(head.strip())
    ...
    if confidence >= 0.9:
        # Known-vendor match — definitely don't need OCR.
        return vendor, confidence, combined
    # Low-confidence heuristic hit from extracted text: return it but let
    # the caller decide whether to fall back to OCR.
    return vendor, confidence, combined
...
def detect_vendor_from_header(
    ...
    """
    Slow path: rasterize + OCR the top portion of the first page(s).
    Only called when fast text extraction did not find a known vendor.

    Uses 150 DPI (down from 300) — sufficient to read a company name and
    roughly 4x faster to rasterize.
    ...
    for page_idx in range(min(pages_to_check, _get_page_count(pdf_path))):
        header_img = extract_header_image(
            ...
            dpi=150,  # intentionally lower than full-page OCR DPI
        )
        if header_img is None:
            continue

        # NOTE: Do NOT binarize header images — vendor logos are often coloured
        # images that Otsu thresholding destroys.  Send the raw RGB crop to
        # PaddleOCR which handles colour/grayscale images natively.
...
def detect_vendor(
    ...
    """
    Master vendor detection entry point.

    Order (fastest to slowest):
      1. Text scan of already-extracted pages (free — no I/O, no OCR).
         Returns immediately on a known-vendor match (conf >= 0.9).
      2. Header OCR fallback — rasterize at 150 DPI and re-run the same
         text parser.  Only reached for scanned / image-only PDFs where
         Step 1 yielded nothing useful.

    Returns:
        (vendor_name, confidence, raw_snippet)
    """
    # ── Fast path: extracted text ──────────────────────────────────────────
    vendor, conf, snippet = detect_vendor_from_text(pages, pages_to_check=pages_to_check + 1)
    if conf >= 0.9:
        logger.debug(f"[vendor] Text-scan hit: {vendor} (conf={conf:.2f})")
        return vendor, conf, snippet

    logger.debug(
        f"[vendor] Text-scan conf={conf:.2f} ({vendor!r}) — falling back to header OCR"
    )

    # ── Slow path: header OCR ──────────────────────────────────────────────
    ocr_vendor, ocr_conf, ocr_snippet = detect_vendor_from_header(
        pdf_path, ocr_cfg, pages_to_check=pages_to_check
    )

    # If OCR found a known vendor, prefer it.
    if ocr_conf >= conf:
        return ocr_vendor, ocr_conf, ocr_snippet

    # Otherwise keep whatever text-scan returned (even low-conf is better
    # than "Unknown" if OCR also failed).
    return vendor, conf, snippet
